...
export default function PackagesPage({ user }: { user?: CurrentUser }) {
  ...
  const remove = useMutation({ mutationFn: (id: string) => deleteData(`/packages/${id}`), onSuccess: () => { message.success('课程方案已删除。'); client.invalidateQueries({ queryKey: ['packages'] }); client.invalidateQueries({ queryKey: ['dashboard'] }); }, onError: (error: Error) => message.error(error.message || '删除课程方案失败，请稍后重试。') });
