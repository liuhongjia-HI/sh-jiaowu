import { Alert, Button, Card, Form, Popconfirm, Skeleton, Space, Table, Typography, message } from 'antd';
import { DeleteOutlined, DownloadOutlined, EditOutlined, EyeOutlined, PlusOutlined, ReloadOutlined, UploadOutlined } from '@ant-design/icons';
...
import type { Course, CurrentUser, Homework, HomeworkSubmissionSummary, LearningSpace, Material, QuestionBankItem, StudyPackage } from '../../types/starline';
...
export function ContentResourcesPage({ kind, user, courseId, packageId, onClearFilter }: { kind: ResourceKind; user?: CurrentUser; courseId?: string; packageId?: string; onClearFilter?: () => void }) {
  ...
  const filterDescription = selectedCourse
    ? `正在查看“${selectedCourse.name}”的全部学习资料。`
    : selectedPackage
      ? `正在查看“${selectedPackage.name}”套餐包含的全部学习资料。`
