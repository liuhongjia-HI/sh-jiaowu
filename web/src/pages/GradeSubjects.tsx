...
export default function GradeSubjects() {
  ...
  const save = useMutation({
    mutationFn: (next: GradeSubjectMetadata) => {
      const all = (items.data ?? []).map((item) => item.id === next.id ? next : item);
  ...
  return <div className="page-stack"><div className="page-heading"><div><h2>年级课程目录</h2><p>维护 G1–G12 每个年级向学生展示的学科、排序和课程图片。学生端始终按这里的目录展示，不由套餐数量决定。</p></div></div><Card><Table rowKey="id" pagination={false} dataSource={items.data} columns={[
  ...
  <FormDrawer title={editing ? `编辑${editing.grade}${editing.displayName}` : '编辑年级课程'} open={Boolean(editing)} onCancel={() => setEditing(null)} onSubmit={() => form.submit()} submitting={save.isPending || upload.isPending}>
    <Form form={form} layout="vertical" onFinish={(values) => editing && save.mutate({ ...editing, ...values, gradeCode: editing.gradeCode })}>
      <Form.Item name="grade" label="年级" rules={[{ required: true }]}><Select disabled options={gradeOptions()} /></Form.Item>
      <Form.Item name="subject" label="学科" rules={[{ required: true }]}><Select disabled options={subjectOptions} /></Form.Item>
