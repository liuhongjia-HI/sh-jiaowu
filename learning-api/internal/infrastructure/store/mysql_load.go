...
func (s *MemoryStore) loadStudentsFromDB() error {
	rows, err := s.db.Query(`SELECT id, name, nickname, avatar_url, grade, phone, school_name, guardian_name, official_account_open_id, account_status, registration_source, remark, learning_status, streak_days, average_score, badge_count, bind_status, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s'), last_study_at, effective_until, enrollment_academic_year, enrollment_grade FROM students ORDER BY id`)
	if err != nil {
		return err
	}
	defer rows.Close()
...
func (s *MemoryStore) loadNoticesFromDB() error {
	rows, err := s.db.Query(`SELECT id, external_id, notice_type, title, target, content, channel, recipient_open_id, status, failure_reason, related_type, related_id, recipient_student_id, retry_count, is_read FROM notices ORDER BY id`)
	if err != nil {
		return err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&dbID, &item.ID, &item.Type, &item.Title, &item.Target, &item.Summary, &item.Channel, &item.RecipientOpenID, &item.Status, &item.FailureReason, &item.RelatedType, &item.RelatedID, &item.RecipientStudentID, &item.RetryCount, &item.IsRead); err != nil {
			return err
		}
		if item.ID == "" {
