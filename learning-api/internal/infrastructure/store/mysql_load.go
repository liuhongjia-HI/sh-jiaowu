...
func (s *MemoryStore) loadStudentsFromDB() error {
	rows, err := s.db.Query(`SELECT id, name, nickname, avatar_url, grade, phone, school_name, guardian_name, official_account_open_id, account_status, remark, learning_status, streak_days, average_score, badge_count, bind_status, last_study_at, effective_until, enrollment_academic_year, enrollment_grade FROM students ORDER BY id`)
	if err != nil {
		return err
	}
	defer rows.Close()
