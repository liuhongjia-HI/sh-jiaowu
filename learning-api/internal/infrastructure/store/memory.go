...
// academicCalendarTerm 是校历上的一个学期条目：某学年某学期的起止日期及期中节点。
...
func defaultSettings() map[string]string {
	...
	calendar, _ := json.Marshal([]academicCalendarTerm{
		{AcademicYear: academicYear, Semester: "S1 第一学期", StartDate: fmt.Sprintf("%d-09-01", startYear), MidtermDate: fmt.Sprintf("%d-11-01", startYear), EndDate: fmt.Sprintf("%d-01-15", startYear+1)},
		{AcademicYear: academicYear, Semester: "S2 第二学期", StartDate: fmt.Sprintf("%d-02-01", startYear+1), MidtermDate: fmt.Sprintf("%d-04-30", startYear+1), EndDate: fmt.Sprintf("%d-07-15", startYear+1)},
	})
	return map[string]string{
		...
		"grades":                       "G1-G12",
		"semesters":                    "S1 / S2",
		"watermarkRule":                "学生专属：姓名/昵称 + STARLINE + 生成日期（服务端写入）",
