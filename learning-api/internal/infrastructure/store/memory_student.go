...
func isTestStudent(student learning.Student) bool {
	...
	return strings.HasPrefix(student.ID, "test-") || strings.Contains(text, "接口测试") || strings.Contains(text, "另外一个孩子") || strings.Contains(text, "一个家长两个孩子") || strings.Contains(text, "王者荣耀") || strings.Contains(text, "王同学") || student.ID == "stu-001" || student.ID == "stu-002" || student.ID == "stu-003"
}
