...
func New(dep Dependencies) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		admin := api.Group("", middleware.AuthRequired(tokens, dep.Service, learning.RoleTeacher, learning.RoleOpsStaff, learning.RoleCampusAdmin, learning.RoleSuperAdmin))
		{
			...
			admin.GET("/homework/:id/submissions", h.HomeworkSubmissions)
			...
			admin.POST("/students/:id/scores", h.CreateStudentScore)
			admin.PUT("/students/:id/scores/:scoreId", h.UpdateStudentScore)
			admin.GET("/notices", h.Notices)
			admin.POST("/notices", h.CreateNotice)
			admin.POST("/notices/:id/retry", h.RetryNotice)
		...
		systemAdmin := api.Group("", middleware.AuthRequired(tokens, dep.Service, learning.RoleCampusAdmin, learning.RoleSuperAdmin))
		{
			...
			systemAdmin.GET("/system/readiness", h.SystemReadiness)
		...
		student := api.Group("/student", middleware.AuthRequired(tokens, dep.Service, learning.RoleStudent))
		{
			...
			student.GET("/materials/:id/preview", h.StudentMaterialPreview)
			student.GET("/materials/:id/preview/pages", h.StudentMaterialPreviewPages)
			student.GET("/materials/:id/preview/pages/:page", h.StudentMaterialPreviewPage)
			student.GET("/homework/:id", h.StudentHomeworkDetail)
			student.POST("/security/events", h.StudentSecurityEvent)
			student.GET("/tasks", h.StudentTasks)
			student.GET("/notices", h.StudentNotices)
			student.GET("/me", h.StudentMe)
			student.PUT("/profile", h.UpdateStudentProfile)
			student.POST("/profile/avatar", h.UploadStudentAvatar)
			...
			student.GET("/scores", h.StudentOwnScores)
