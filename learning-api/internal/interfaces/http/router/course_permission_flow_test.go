...
func TestCoursePermissionActivationAndRevocationFlow(t *testing.T) {
	...
	for _, lesson := range []string{"-lesson-1", "-lesson-2", "-lesson-3"} {
		...
		homework, err := app.store.CreateHomework("测试", principal, learning.HomeworkUploadRequest{Title: lesson + "习题", CourseID: courseID, LessonID: courseID + lesson, TagCode: "TK", Status: string(learning.StatusEnabled)})
		if err != nil {
			t.Fatal(err)
		}
