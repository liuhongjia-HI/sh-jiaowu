...
if command -v gs >/dev/null 2>&1; then
	if [ ! -f "$watermark_font_path" ]; then
		...
		printf '%s\n' '%!PS' "/StarlineNotoSansCJKsc << /FileType /TrueType /Path ($watermark_font_path) /SubfontID 2 /CSI [(GB1) 2] >> ;" > "$watermark_cidfmap"
		...
		if ! gs -q -dBATCH -dNOPAUSE -dNODISPLAY \
			...
			-c '/StarlineNotoSansCJKsc-UniGB-UTF16-H findfont pop quit' >/dev/null 2>&1; then
			missing+=("Noto CJK GB1 Unicode Ghostscript font map")
		fi
