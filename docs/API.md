...
学科颜色、简称和排序属于学科元数据，不放入系统设置 JSON：`GET /api/subjects` 获取完整列表，校区管理员和超级管理员可通过 `PUT /api/subjects/{id}` 维护 `shortLabel`、`color`、`sortOrder` 与 `status`。学科名称不在此接口修改，避免破坏已有课程、学习空间和历史数据的关联。列表项带只读字段 `deletable`；仅当学科不是系统内置、且未被课程、学习空间、年级目录等业务数据引用时为 `true`。校区管理员和超级管理员可通过 `DELETE /api/subjects/{id}` 删除这类残留学科。内置学科（含已停用的综合科学、历史）不能删除，如不再开设请改为停用。
