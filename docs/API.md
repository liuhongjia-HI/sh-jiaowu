...
返回体在此基础上附带 `academicYear` / `semester`：按 `startDate` 落系统设置里的校历（`GET/PUT /api/settings` 的 `academicCalendar`）判定一次后固定写入，此后调校历、切学年都不会让已排课程的归属跟着变；只有真的改了开课日期或课程才会重新判定。每条校历记录除 `startDate`、`endDate` 外，还可维护 `midtermDate`、`finalDate`；两者必须落在对应学期内，且期中不得晚于期末。开课日期落不进任何校历学期时（例如假期班），退回课程所属学习空间的学期标签 + 开课日期本身的自然年 7 月 1 日规则，不阻塞建班。
...
使用 `公众号模板消息` 时，后端会同时生成一条同内容的 `站内通知`，供后台对照发送记录；这条站内副本不再进入学生端通知页。公众号模板消息记录用于后台查看发送状态、失败原因和补发次数。
...
误给某名学生开通套餐时，可调用 `DELETE /api/grants/{packageId}?studentId=stu-001` 撤销该学生的这项套餐权限。此操作只收回该套餐覆盖的课程、讲义和练习，不会删除共享套餐，也不会影响其他学生；单独开通的内容仍应通过 `PUT /api/grants/direct` 调整。
...
多子女家庭的静默登录可额外携带 `selectedStudentId`。服务端只会在该家长仍有效关联的学生范围内恢复当前孩子，适合不同设备分别记住各自正在查看的孩子。
...
新增、续期和扩大开通权限须满足：学科存在于 `subjects` 且启用、空间存在且启用、学生当前年级匹配、学科及等级符合该年级规则。直接开通及套餐预览/开通共用后端校验；年级展示目录不作为授权白名单。

学生详情的 `openingMatrix` 只返回可新增范围，每项包含 `subjectId`、`subjectSortOrder` 和规范 `subject`。`grants` 保留旧记录，不可新增范围的授权附带 `openingBlockedReason`；有效期及学生端已有权限不会因为学科启停而自动改变。

`PUT /api/grants/direct` 的 `selections` 替换当前可新增范围内的直接开通。空数组只清空这部分权限；无法新增的历史直接授权默认保留。可选 `revokeDirectLearningSpaceIds: string[]` 明确撤销指定学生的历史直接授权，不得与选中项冲突，不影响套餐权限。已到期记录不可用该字段重新开通。

旧客户端若原样回传不可新增范围的未到期直接权限，服务端只保留原记录，不调整其日期；新增或修改此类权限会整次失败。公共 `startsAt/endsAt` 只应用于当前可新增且选中的范围。校验失败不提交任何部分变更。
