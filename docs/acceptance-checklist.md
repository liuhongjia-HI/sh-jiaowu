...
- `/scheduling` 首屏显示 `排班工作台`，默认进入按老师查看的日视图。
- 日、工作周、周、月、列表视图可以切换；没有课程时仍保留时间轴和新建入口。
- 支持上一天/周、下一天/周、今天、小月历选日期。
- 左侧显示学科日历和筛选项。
- 有正式课程时，课程块按时间定位；可上课时间作为辅助条，不压缩正式课程。
- 无课程时显示可新建入口和轻提示。
- 页面标题下明确提示“右键课程可快速复制”。
- 日、周、月视图右键有效课程后显示“复制这节课”；复制抽屉标题为“复制课程”。
- 复制表单自动带入课程、老师、学生、校区、班型、课长、日期、时间、预计人数和预留说明，用户可只修改日期或时间后创建。
- 复制创建的是独立新课，不改变原课程，也不继承原课程 ID、系列身份和审核状态；老师复制仍进入待审核。
- 复制提交仍执行老师/学生撞课、容量、年级、学科权限和可上课时间校验。
- 新建课程可开启重复排课，支持每 N 天、每 N 周以及每周多个上课日。
- 重复规则可按总课次数或结束日期结束；单次最多生成 200 节，任一课次冲突时整批不创建。
- 界面明确说明固定周次通过“每 N 周”设置，按月和特殊日期重复将在后续开放。
- 教师只能给自己排课；教师创建或复制的课程进入待审核，管理员通过前不出现在学生课表、首页待办和学生通知中。
- 运营教务、校区管理员、超级管理员可通过或驳回排课；教师不能审核自己的课程，驳回必须填写理由。
- 教师修改被驳回课程后重新进入待审核，旧驳回理由和审核人不继续作为当前审核结论；已通过课程只能联系管理员调整。
- 教师维护的全局可上课时间仅作为匹配与监管范围；越界排课需二次确认、保留越界说明，并在审核队列展示。
...
4. 在已有课程块上点击右键，选择“复制这节课”。
5. 确认抽屉标题、预填内容和“创建复制课程”按钮正确；只修改日期或时间后提交。
6. 确认新课创建成功、原课程未变化；原样时间提交时应提示撞课。
7. 新建课程时开启重复排课，分别检查“按日”和“按周”；按周设置“每 2 周、周一和周三、共 6 节”并提交。
8. 确认生成课次共享同一个 `seriesId`，日期符合固定周次规则；月度重复请求应被明确拒绝。
9. 使用教师账号创建课程，确认只能选择自己且返回待审核；学生课表、首页待办和通知中均不可见。
10. 使用运营教务或管理员账号通过该课程，确认学生端开始可见；另建一节并驳回，确认必须填写理由。
11. 教师修改被驳回课程，确认重新进入待审核且管理员可再次处理；直接构造其他 `teacherId` 的请求应被后端拒绝。
12. 将课程排在教师全局可上课时间之外，确认需要二次确认，审核列表能看到越界提示。
13. 全页搜索教室/地点相关文案。

整改建议：
- 时间轴不显示：确认空数据时也走时间轴渲染，而不是整页空状态。
- 课程块重叠：检查 `layoutOverlappingItems` 和时间字段格式。
- 右键没有复制菜单：检查日/周/月课程块的 `contextMenu` 和当前账号排课权限。
- 复制后字段为空：检查 `openCopy` 的表单预填映射；不要直接复制数据库记录。
